...
int main(int argc, char* argv[]) {
    ...
    // criação do pc inicializando em offset
    uint32_t pc = offset;
    ...
    uint32_t endereco_atual = offset;
    char linha[76];
    ...
    while (fgets(linha, sizeof(linha), input)) {
        if (linha[0] == '@') {
            ...
            while (sscanf(linha + index, "%2x", &byte) == 1) {
                mem[endereco_atual - offset] = (uint8_t) byte;
                endereco_atual++;
                index += 3;
    ...
    while (run) {
        ...
        uint32_t instruction = ((uint32_t*) mem)[(pc - offset) >> 2];
        ...
        // leitura do campo imm com sinal
        int32_t imm_i = ((int32_t)(instruction)) >> 20;
        // leitura do campo imm sem sinal
        ...
        switch (opcode) {
            ...
            case 0b0010011:
                ...
                /* Compara o registrador rs1 com o imediato (imm) como valores sem sinal. */
                    ...
                    uint32_t data = (reg[rs1] < imm_u) ? 1 : 0;
                ...
                /* Compara o registrador rs1 com o imediato (imm) como valores com sinal. */
                    ...
                    uint32_t data = ((int32_t)reg[rs1] < imm_i) ? 1 : 0;
            ...
            case 0b0000011:
                ...
                if (funct3 == 0b000) {
                    int8_t data = *((int8_t*)&mem[reg[rs1] + imm_i]);
                ...
                /* Carrega 2 bytes (16 bits) da memória para um registrador. */
                    ...
                    int16_t data = *((int16_t*)&mem[reg[rs1] + imm_i]);
                ...
                /* Carrega 2 bytes (16 bits) da memória e zero-extende para 32 bits. */
                    ...
                    uint16_t data = *((uint16_t*)&mem[reg[rs1] + imm_i]);
                ...
                if (funct3 == 0b000) {
                    uint8_t data = (uint8_t)(reg[rs2] & 0xFF);
                    *((uint8_t*)&mem[reg[rs1] + imm_s]) = data;
                ...
                /* Armazena os 16 bits menos significativos do registrador rs2 na memória. */
                    ...
                    uint16_t data = (uint16_t)(reg[rs2] & 0xFFFF);
                    *((uint16_t*)&mem[reg[rs1] + imm_s]) = data;
